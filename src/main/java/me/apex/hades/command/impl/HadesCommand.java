                    ...
                    if (input.args()[0].equalsIgnoreCase("info")){
                        if(input.args().length > 1){
                            ...
                            if (target != null){
                                ...
                                user.sendMessage(HadesConfig.BASE_MESSAGE_COLOR + "IP Address: &f" + user.getPlayer().getAddress().getAddress().toString().replace("/", ""));
                        ...
                        if (player.hasPermission(HadesConfig.BASE_PERMISSION + ".alerts")) {
                            if(input.args().length > 1) {
                                try{
                                    int flagDelay = Integer.valueOf(input.args()[1]);
                                    user.setAlerts(true);
                                    ...
                                    if(input.args()[1].equalsIgnoreCase("dev")) {
                                        ...
                                        user.setFlagDelay(1);
                                        user.sendMessage(ChatUtil.color(user.isAlerts() ? HadesConfig.ENABLE_ALERTS_MESSAGE.replace("%flagsetting%", "" + 1) : HadesConfig.DISABLE_ALERTS_MESSAGE));
                                    }
                                }
                            }else {
                                user.setAlerts(!user.isAlerts());
