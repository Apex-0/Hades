    ...
    public void onHandle(PacketEvent e, User user) {
        if (e instanceof FlyingEvent) {
            if (user.getTick() > 10 && elapsed(user.getTick(), user.getFlyingTick()) > 20) {
                ...
                if (delta >= (user.getPlayer().getWalkSpeed() > 0.2f ? .23 * 1 + ((user.getPlayer().getWalkSpeed() / 0.2f) * 0.36) : 0.23 + (PlayerUtils.getPotionEffectLevel(data.getPlayer(), PotionEffectType.SPEED) * 0.062f)) && PlayerUtil.isOnGround(user.getPlayer()) && user.isSprinting() && user.getDeltaXZ() > 0.1 && !user.isInLiquid() && !user.isInWeb()) {
                    if (++preVLA > 4) {
