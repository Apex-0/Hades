...
public abstract class Check implements Listener {
    ...
    protected void flag(User user, String type, String information, boolean dev) {
        ...
        //Alert Message
        TextComponent alertMessage = new TextComponent(alertFormatColor.replace("%prefix%", HadesConfig.PREFIX).replace("%player%", user.getPlayer().getName()).replace("%check%", getName()).replace("%checktype%", type).replace("%vl%", String.valueOf(vl)).replace("%info%", information).replace("%ping%", user.ping() + "").replace("%tps%", PacketEvents.getAPI().getServerUtils().getCurrentTPS() + "") + (dev ? " §7(Exp)" : ""));
